const database = [
  {
    quizId: "dia-li-chu-de-4",
    title: "CHỦ ĐỀ 4: THIÊN NHIÊN NHIỆT ĐỚI ẨM GIÓ MÙA",
    timeMinutes: 90,
    questions: [
      {
        id: 1,
        question: "Tính chất nhiệt đới của khí hậu nước ta được quy định bởi vị trí",
        options: {
          A: "nằm ở vùng nội chí tuyến bán cầu Bắc.",
          B: "tiếp giáp vùng biển nhiệt đới rộng lớn.",
          C: "nằm tại nơi giao tranh của các khối khí.",
          D: "nằm trong hoàn lưu gió mùa châu Á."
        },
        correctAnswer: "A"
      },
      {
        id: 2,
        question: "Biểu hiện nào sau đây không thể hiện tính chất nhiệt đới của khí hậu nước ta?",
        options: {
          A: "Số giờ nắng trong năm cao.",
          B: "Nhiệt độ phổ biến trên 20°C",
          C: "Lượng bức xạ mặt trời lớn.",
          D: "Cân bằng ẩm luôn dương."
        },
        correctAnswer: "D"
      },
      {
        id: 3,
        question: "Tính chất ẩm của khí hậu nước ta được thể hiện ở",
        options: {
          A: "mưa nhiều, lượng bức xạ mặt trời lớn.",
          B: "lượng mưa lớn, độ ẩm tương đối cao.",
          C: "nguồn ẩm lớn, mưa đều khắp cả nước.",
          D: "mưa nhiều quanh năm, độ ẩm khá lớn."
        },
        correctAnswer: "B"
      },
      {
        id: 4,
        question: "Nước ta có lượng mưa hàng năm lớn chủ yếu do",
        options: {
          A: "địa hình có nhiều đồi núi thấp.",
          B: "nhiệt độ cao, lượng bốc hơi lớn.",
          C: "các khối khí di chuyển qua biển.",
          D: "Tín phong hoạt động suốt năm."
        },
        correctAnswer: "C"
      },
      {
        id: 5,
        question: "Những nơi mưa lớn ở nước ta thường do ảnh hưởng của",
        options: {
          A: "địa hình đón gió ẩm.",
          B: "vị trí nằm ở gần biển.",
          C: "độ cao địa hình lớn.",
          D: "hoạt động gió mùa."
        },
        correctAnswer: "A"
      },
      {
        id: 6,
        question: "Tác động của gió mùa đối với khí hậu cả nước là",
        options: {
          A: "có một mùa nhiệt độ hạ thấp.",
          B: "lượng mưa thay đổi theo mùa.",
          C: "khí hậu ổn định và ít thiên tai.",
          D: "nhiệt ẩm giống nhau ở các nơi."
        },
        correctAnswer: "B"
      },
      {
        id: 7,
        question: "Tín phong bán cầu Bắc thổi vào nước ta theo hướng",
        options: {
          A: "đông nam.",
          B: "đông bắc.",
          C: "tây bắc.",
          D: "tây nam."
        },
        correctAnswer: "B"
      },
      {
        id: 8,
        question: "Từ tháng XI đến tháng IV năm sau, đồng bằng Bắc Bộ có gió mùa Đông Bắc hoạt động xen kẽ với",
        options: {
          A: "gió thổi từ Xích đạo tới.",
          B: "gió từ vịnh Ben-gan tới.",
          C: "gió tây nam khô nóng.",
          D: "Tín phong bán cầu Bắc."
        },
        correctAnswer: "D"
      },
      {
        id: 9,
        question: "Về mùa đông, loại gió chiếm ưu thế từ Đà Nẵng trở vào nam là",
        options: {
          A: "gió mùa Tây Nam.",
          B: "gió mùa Đông Bắc.",
          C: "Tín phong bán cầu Nam.",
          D: "Tín phong bán cầu Bắc."
        },
        correctAnswer: "D"
      },
      {
        id: 10,
        question: "Tín phong hoạt động ở nước ta chủ yếu do nước ta có vị trí",
        options: {
          A: "gần với các áp cao trên lục địa.",
          B: "nằm hoàn toàn ở bán cầu Bắc.",
          C: "nằm trong vùng nội chí tuyến.",
          D: "nằm ở khu vực châu Á gió mùa."
        },
        correctAnswer: "C"
      },
      {
        id: 11,
        question: "Khu vực ven biển miền Trung có",
        options: {
          A: "mùa mưa lệch về thu đông.",
          B: "mưa nhiều vào đầu mùa hạ.",
          C: "nhiệt độ thấp nhất cả nước.",
          D: "khô hạn trong cả mùa đông."
        },
        correctAnswer: "A"
      },
      {
        id: 12,
        question: "Vào thời kì gió đông bắc, Nam Bộ có",
        options: {
          A: "mùa khô rất sâu sắc, thời tiết nóng ẩm.",
          B: "mùa hạ kéo dài, nhiều mưa và có bão.",
          C: "mùa khô kéo dài, thời tiết nắng nóng.",
          D: "mùa đông kéo dài, nhiệt độ khá thấp."
        },
        correctAnswer: "C"
      },
      {
        id: 13,
        question: "Tín phong bán cầu Bắc ở nước ta",
        options: {
          A: "mạnh vào các tháng cuối năm, khô nóng.",
          B: "tạo nên dải hội tụ, đem lại lượng mưa lớn.",
          C: "hoạt động quanh năm, xen kẽ với gió mùa.",
          D: "thổi ở phạm vi cả nước, tính chất nóng ẩm."
        },
        correctAnswer: "C"
      },
      {
        id: 14,
        question: "Ở nước ta, Tín phong bán cầu Bắc đã tạo ra",
        options: {
          A: "một mùa mưa ít, nhiệt độ thấp cho Nam Bộ.",
          B: "mưa lớn quanh năm cho ven biển Trung Bộ.",
          C: "mùa khô sâu sắc, kéo dài cho miền núi Bắc Bộ.",
          D: "thời tiết ấm áp vào mùa đông cho vùng Bắc Bộ."
        },
        correctAnswer: "D"
      },
      {
        id: 15,
        question: "Gió mùa Đông Bắc nước ta có đặc điểm là",
        options: {
          A: "thổi liên tục trong suốt mùa đông.",
          B: "bị chặn hoàn toàn ở dãy Bạch Mã.",
          C: "hoạt động mạnh nhất ở Đông Bắc.",
          D: "hoạt động quanh năm ở miền Bắc."
        },
        correctAnswer: "C"
      },
      {
        id: 16,
        question: "Gió mùa mùa đông ở nước ta",
        options: {
          A: "gây ra một mùa mưa cho Tây Nguyên.",
          B: "bị suy yếu khi di chuyển về phía nam.",
          C: "hoạt động liên tục trong suốt mùa đông.",
          D: "gây mưa ở cả hai sườn dãy Trường Sơn."
        },
        correctAnswer: "B"
      },
      {
        id: 17,
        question: "Khối không khí lạnh hoạt động trong mùa đông ở nước ta",
        options: {
          A: "có nguồn gốc từ áp cao Bắc Ấn Độ Dương.",
          B: "làm hạ thấp nhiệt độ ở đồng bằng Bắc Bộ.",
          C: "không gây mưa ở phần lãnh thổ phía Bắc.",
          D: "hoạt động liên tục, kéo dài trong suốt mùa."
        },
        correctAnswer: "B"
      },
      {
        id: 18,
        question: "Nửa sau mùa đông, gió mùa Đông Bắc gây mưa phùn cho đồng bằng Bắc Bộ chủ yếu do",
        options: {
          A: "thay đổi vị trí xuất phát.",
          B: "đi qua lục địa Trung Hoa.",
          C: "đi qua vùng biển rộng lớn.",
          D: "gặp sườn núi đón gió biển."
        },
        correctAnswer: "C"
      },
      {
        id: 19,
        question: "Vùng khí hậu Bắc Trung Bộ có mưa vào mùa đông do dãy Trường Sơn Bắc tương tác với",
        options: {
          A: "gió từ vịnh Ben-gan tới, gió từ bán cầu Nam.",
          B: "gió từ áp cao Xi-bia, Tín phong bán cầu Bắc.",
          C: "gió mùa Đông Bắc, Tín phong bán cầu Nam.",
          D: "gió từ áp cao Bắc Ấn Độ Dương, gió đông bắc."
        },
        correctAnswer: "B"
      },
      {
        id: 20,
        question: "Gió mùa Đông Bắc có thể xâm nhập sâu vào miền Bắc nước ta chủ yếu do",
        options: {
          A: "vị trí nằm ở xa vùng xích đạo.",
          B: "địa hình chủ yếu đồi núi thấp.",
          C: "hướng các dãy núi ở Đông Bắc.",
          D: "nằm gần vị trí của áp cao Xibia."
        },
        correctAnswer: "C"
      },
      {
        id: 21,
        question: "Đặc điểm chung của hai loại gió cùng hoạt động trên lãnh thổ nước ta trong mùa đông là",
        options: {
          A: "làm nhiệt độ tăng, lượng mưa giảm dần về phía nam.",
          B: "hướng đông bắc là chính, hoạt động mạnh ở Bắc Bộ.",
          C: "gây mưa cho ven biển Trung Bộ, thổi thành từng đợt.",
          D: "tính chất khô, đem lại lượng mưa ít trên nhiều khu vực."
        },
        correctAnswer: "D"
      },
      {
        id: 22,
        question: "Nam Bộ hầu như không chịu ảnh hưởng của gió mùa Đông Bắc chủ yếu do",
        options: {
          A: "Tín phong bán cầu Bắc mạnh, các cao nguyên cao.",
          B: "độ cao địa hình, gió mùa Đông Bắc bị suy yếu dần.",
          C: "vị trí nằm gần vùng xích đạo, các dãy núi chắn gió.",
          D: "vùng biển phía Nam nóng ẩm, đồng bằng rộng lớn."
        },
        correctAnswer: "C"
      },
      {
        id: 23,
        question: "Về mùa hạ, Bắc Bộ có",
        options: {
          A: "nhiệt độ hạ thấp, mưa ít.",
          B: "lạnh ẩm, độ ẩm khá cao.",
          C: "nền nhiệt cao, mưa nhiều.",
          D: "mưa phùn, nhiệt độ cao."
        },
        correctAnswer: "C"
      },
      {
        id: 24,
        question: "Ở nước ta, gió phơn hoạt động chủ yếu ở",
        options: {
          A: "phía bắc của Tây Bắc.",
          B: "Đông Bắc Bộ, Bắc Bộ.",
          C: "Tây Nguyên, Nam Bộ.",
          D: "duyên hải miền Trung."
        },
        correctAnswer: "D"
      },
      {
        id: 25,
        question: "Gió phơn ở nước ta",
        options: {
          A: "hoạt động liên tục vào đầu mùa hạ.",
          B: "gây mưa nhiều cho dãy Trường Sơn.",
          C: "tác động mạnh ở ven biển Trung Bộ.",
          D: "thổi chủ yếu theo hướng đông nam."
        },
        correctAnswer: "C"
      },
      {
        id: 26,
        question: "Dải hội tụ nội chí tuyến ở nước ta",
        options: {
          A: "có khả năng gây mưa lớn.",
          B: "gây ra một mùa đông lạnh.",
          C: "hoạt động trong suốt năm.",
          D: "tạo mùa khô dài và sâu sắc."
        },
        correctAnswer: "A"
      },
      {
        id: 27,
        question: "Loại gió hoạt động vào đầu mùa hạ ở nước ta không có đặc điểm nào sau đây?",
        options: {
          A: "Gây khô nóng cho ven biển Trung Bộ.",
          B: "Thổi vào nước ta theo hướng tây nam.",
          C: "Có nguồn gốc từ Nam Ấn Độ Dương.",
          D: "Gây mưa lớn cho đồng bằng Nam Bộ."
        },
        correctAnswer: "C"
      },
      {
        id: 28,
        question: "Loại gió hoạt động vào giữa và cuối mùa hạ ở nước ta",
        options: {
          A: "gây khô nóng cho ven biển Trung Bộ.",
          B: "gây mưa lớn, kết thúc sớm ở Nam Bộ.",
          C: "có bản chất là Tín phong bán cầu Nam.",
          D: "thổi vào Bắc Bộ theo hướng tây nam."
        },
        correctAnswer: "C"
      },
      {
        id: 29,
        question: "Ở nước ta, gió tây nam thổi từ Bắc Ấn Độ Dương tới đã",
        options: {
          A: "làm xuất hiện gió phơn tây nam ở Nam Bộ.",
          B: "gây mưa lớn, kéo dài ven biển Nam Trung Bộ.",
          C: "tạo nên một mùa khô sâu sắc cho Tây Nguyên.",
          D: "làm mùa mưa ở Bắc Trung Bộ đến muộn hơn."
        },
        correctAnswer: "D"
      },
      {
        id: 30,
        question: "Gió mùa Tây Nam ở nước ta",
        options: {
          A: "thổi vào miền Bắc theo hướng tây bắc.",
          B: "hoạt động kéo dài ở khu vực Nam Bộ.",
          C: "hoạt động liên tục trong suốt mùa hạ.",
          D: "chỉ gây mưa cho phía tây Trường Sơn."
        },
        correctAnswer: "B"
      },
      {
        id: 31,
        question: "Khối khí nhiệt đới ẩm từ Bắc Ấn Độ Dương",
        options: {
          A: "di chuyển vào nước ta liên tục theo hướng tây nam.",
          B: "gây ra thời tiết khô nóng cho vùng Đông Nam Bộ.",
          C: "hoạt động thành từng đợt vào thời gian đầu mùa hạ.",
          D: "đem lại lượng mưa lớn cho các đồng bằng châu thổ."
        },
        correctAnswer: "C"
      },
      {
        id: 32,
        question: "Phần lớn lãnh thổ nước ta có nhiệt độ trung bình năm cao, vượt tiêu chuẩn khí hậu nhiệt đới chủ yếu do",
        options: {
          A: "tiếp giáp với Biển Đông, có nhiều đồng bằng châu thổ sông rộng.",
          B: "vị trí trong nội chí tuyến, đồi núi thấp chiếm phần lớn diện tích.",
          C: "gió tây nam từ Bắc Ấn Độ Dương đến, vị trí ở khu vực gió mùa.",
          D: "nằm trong vùng xích đạo, tiếp nhận nguồn bức xạ Mặt trời lớn."
        },
        correctAnswer: "B"
      },
      {
        id: 33,
        question: "Bắc Bộ có lượng mưa thay đổi trong năm chủ yếu do tác động của",
        options: {
          A: "gió đông bắc, gió tây nam, địa hình thấp và đón gió.",
          B: "gió tây nam từ vịnh Bengan đến, Biển Đông, frông.",
          C: "gió mùa hạ, gió mùa đông, dải hội tụ nhiệt đới, bão.",
          D: "gió mùa Tây Nam, Tín phong bán cầu Bắc, vị trí địa lí."
        },
        correctAnswer: "C"
      },
      {
        id: 34,
        question: "Chế độ mưa ở vùng khí hậu Bắc Trung Bộ chủ yếu do tác động kết hợp của các nhân tố nào sau đây?",
        options: {
          A: "Gió mùa Tây Nam, gió đông bắc, bão và dải hội tụ nhiệt đới, hướng địa hình.",
          B: "Các gió hướng đông bắc, gió mùa Tây Nam, vị trí địa lí, độ cao và hướng núi.",
          C: "Hướng của dãy núi Trường Sơn Bắc, gió, bão, áp thấp và dải hội tụ nhiệt đới.",
          D: "Vị trí địa lí, khối khí lạnh phương Bắc, gió hướng tây nam, frông và dải hội tụ."
        },
        correctAnswer: "C"
      },
      {
        id: 35,
        question: "Gió phơn hoạt động mạnh ở Bắc Trung Bộ chủ yếu do tác động của",
        options: {
          A: "các đỉnh núi và hướng núi, gió tây nam từ vịnh Bengan đến, áp thấp Bắc Bộ.",
          B: "gió mùa Tây Nam, áp cao Bắc Ấn Độ Dương, lãnh thổ hẹp ngang và kéo dài.",
          C: "thảm thực vật ít, gió tây nam trong mùa hạ, hai lần Mặt Trời lên thiên đỉnh.",
          D: "nền nhiệt cao, bốc hơi mạnh, gió tây nam từ Bắc Ấn Độ Dương, áp thấp Bắc Bộ."
        },
        correctAnswer: "A"
      },
      {
        id: 36,
        question: "Dải đồng bằng ven biển Trung Bộ nước ta có thời gian bắt đầu và kết thúc mùa mưa muộn hơn các khu vực khác chủ yếu do tác động của",
        options: {
          A: "gió mùa Tây Nam, gió mùa Đông Bắc, dải hội tụ nhiệt đới, dãy Trường Sơn.",
          B: "Tín phong bán cầu Bắc, gió mùa Tây Nam, dải hội tụ nhiệt đới, bão, áp thấp.",
          C: "gió tây nam từ Bắc Ấn Độ Dương, Tín phong, áp thấp nhiệt đới và dải hội tụ.",
          D: "gió phơn Tây Nam, gió mùa Tây Nam, gió đông bắc, dải hội tụ nhiệt đới, bão."
        },
        correctAnswer: "D"
      },
      {
        id: 37,
        question: "Nguyên nhân chủ yếu gây mưa vào mùa hạ ở nước ta là",
        options: {
          A: "gió mùa hạ, dải hội tụ, bão và áp thấp nhiệt đới.",
          B: "Tín phong bán cầu Bắc, frông, gió mùa Tây Nam.",
          C: "gió tây nam, gió đông bắc, dải hội tụ, Biển Đông.",
          D: "gió mùa Đông Nam, gió tây nam, bão và áp thấp."
        },
        correctAnswer: "A"
      },
      {
        id: 38,
        question: "Bắc Bộ có một mùa mưa nhiều chủ yếu do tác động của",
        options: {
          A: "gió tây nam từ vịnh Bengan đến, Tín phong bán cầu Bắc, dải hội tụ.",
          B: "Tín phong bán cầu Bắc, địa hình đồi núi, áp thấp nhiệt đới, dải hội tụ.",
          C: "gió mùa Tây Nam, gió tây nam từ vịnh Bengan đến, dải hội tụ và bão.",
          D: "gió tây nam, gió Tây, địa hình đồi núi, áp thấp nhiệt đới và dải hội tụ."
        },
        correctAnswer: "C"
      },
      {
        id: 39,
        question: "Vùng khí hậu Bắc Trung Bộ có lượng mưa trong mùa đông lớn hơn vùng khí hậu Tây Bắc Bộ chủ yếu do",
        options: {
          A: "tác dụng chắn của dãy Trường Sơn đối với gió hướng đông bắc, gần biển.",
          B: "chịu ảnh hưởng của luồng gió thổi theo hướng tây nam, nhiệt độ gia tăng.",
          C: "khối khí Bắc Ấn Độ Dương tác động mạnh, gió mùa Đông Bắc bị biến tính.",
          D: "đường bờ biển dài, hoạt động của frông kết hợp với hướng của các dãy núi."
        },
        correctAnswer: "A"
      },
      {
        id: 40,
        question: "Mùa mưa ở dải đồng bằng ven biển Nam Trung Bộ chịu tác động chủ yếu của",
        options: {
          A: "gió mùa Tây Nam, áp thấp nhiệt đới, gió tây nam từ Bắc Ấn Độ Dương đến.",
          B: "gió hướng đông bắc, gió mùa Tây Nam, dải hội tụ, bão và áp thấp nhiệt đới.",
          C: "áp thấp nhiệt đới và bão, gió mùa Tây Nam, gió Tây và gió mùa Đông Bắc.",
          D: "dải hội tụ, Tín phong bán cầu Bắc và gió tây nam từ Bắc Ấn Độ Dương đến."
        },
        correctAnswer: "B"
      },
      {
        id: 41,
        question: "Nguyên nhân chủ yếu gây mưa cho vùng khí hậu Nam Bộ là",
        options: {
          A: "gió tây nam từ Bắc Ấn Độ Dương đến, Tín phong bán cầu Bắc và dải hội tụ.",
          B: "gió mùa Tây Nam, Tín phong bán cầu Bắc, bão, dải hội tụ, áp thấp nhiệt đới.",
          C: "bão, dải hội tụ và áp thấp nhiệt đới, gió mùa Tây Nam và gió phơn Tây Nam.",
          D: "gió mùa Tây Nam, dải hội tụ nhiệt đới, gió tây nam từ Bắc Ấn Độ Dương đến."
        },
        correctAnswer: "D"
      },
      {
        id: 42,
        question: "Vùng khí hậu Nam Trung Bộ có mưa ít vào đầu mùa hạ chủ yếu do",
        options: {
          A: "chuyển động biểu kiến của Mặt Trời và hoạt động của gió Mậu dịch.",
          B: "hoạt động của gió hướng tây nam kết hợp với hướng của các dãy núi.",
          C: "hoạt động của gió mùa Tây Nam và địa hình song song với hướng gió.",
          D: "tác động của Tín phong bán cầu Bắc và địa hình thấp trũng, khuất gió."
        },
        correctAnswer: "B"
      },
      {
        id: 43,
        question: "Tây Nguyên và Nam Bộ có một mùa khô sâu sắc chủ yếu do tác động của",
        options: {
          A: "địa hình thấp, các loại gió đông bắc, hoạt động của frông.",
          B: "Tín phong bán cầu Bắc, vị trí gần xích đạo, bốc hơi mạnh.",
          C: "gió từ vịnh Bengan đến, gió đông bắc, hướng các dãy núi.",
          D: "nằm ở vĩ độ thấp, các loại gió hướng đông bắc, dải hội tụ."
        },
        correctAnswer: "B"
      },
      {
        id: 44,
        question: "Về mùa đông, Bắc Bộ có lượng mưa ít và thời tiết không ổn định chủ yếu do tác động của",
        options: {
          A: "địa hình thấp, vị trí trong khu vực gió mùa, dải hội tụ nhiệt đới.",
          B: "vị trí cách xa xích đạo, Tín phong bán cầu Bắc, áp thấp nhiệt đới.",
          C: "gió mùa Đông Bắc, hoạt động của frông, Tín phong bán cầu Bắc.",
          D: "vị trí giáp với Biển Đông, địa hình cao ở rìa đông bắc, Tín phong."
        },
        correctAnswer: "C"
      },
      {
        id: 45,
        question: "Khí hậu Nam Bộ phân chia thành hai mùa rõ rệt chủ yếu do tác động kết hợp của",
        options: {
          A: "gió mùa Tây Nam, Tín phong bán cầu Bắc và độ cao địa hình.",
          B: "Tín phong Đông Bắc, dải hội tụ nhiệt đới và áp thấp nhiệt đới.",
          C: "Tín phong bán cầu Bắc với địa hình dãy núi Trường Sơn Nam.",
          D: "gió hướng tây nam, Tín phong bán cầu Bắc và hướng địa hình."
        },
        correctAnswer: "D"
      },
      {
        id: 46,
        question: "Biểu hiện của quá trình xâm thực ở nước ta không phải là",
        options: {
          A: "bề mặt địa hình nhiều cắt xẻ.",
          B: "hiện tượng đất trượt và đá lở.",
          C: "địa hình cacxto khá phổ biến.",
          D: "các đồng bằng được mở rộng."
        },
        correctAnswer: "D"
      },
      {
        id: 47,
        question: "Tính chất nhiệt đới ẩm gió mùa trong địa hình nước ta được thể hiện ở",
        options: {
          A: "nhiều sông suối, vịnh nước sâu kín gió.",
          B: "cao nguyên rộng lớn, thung lũng sông.",
          C: "sạt lở đất miền núi, các quần đảo gần bờ.",
          D: "hang động cac-xtơ, đồng bằng mở rộng."
        },
        correctAnswer: "D"
      },
      {
        id: 48,
        question: "Hệ quả của quá trình xâm thực mạnh ở đồi núi nước ta là",
        options: {
          A: "tạo ra các cao nguyên badan xếp tầng.",
          B: "hạ thấp địa hình rìa phía bắc lãnh thổ.",
          C: "mở mang đồng bằng vùng hạ lưu sông.",
          D: "hình thành các bề mặt san bằng rộng."
        },
        correctAnswer: "C"
      },
      {
        id: 49,
        question: "Ở nước ta, vùng hạ lưu sông lớn có quá trình nào sau đây diễn ra mạnh?",
        options: {
          A: "Bồi tụ.",
          B: "Bóc mòn.",
          C: "Xâm thực.",
          D: "Phong hóa."
        },
        correctAnswer: "A"
      },
      {
        id: 50,
        question: "Vào thời kì mưa lớn, các sườn núi dốc có nguy cơ xảy ra hiện tượng",
        options: {
          A: "xói mòn, lũ ống, giá rét.",
          B: "đất trượt, đá lở, lũ quét.",
          C: "nước dâng, bão, ngập lụt.",
          D: "gió mạnh, mưa đá, lũ lụt."
        },
        correctAnswer: "B"
      },
      {
        id: 51,
        question: "Địa hình các-xtơ ở nước ta",
        options: {
          A: "gắn với các khu vực đá mắc-ma.",
          B: "hình thành do khí hậu khô hạn.",
          C: "phân bố toàn bộ tại đồng bằng.",
          D: "có khả năng phát triển du lịch."
        },
        correctAnswer: "D"
      },
      {
        id: 52,
        question: "Quá trình xâm thực diễn ra mạnh mẽ ở vùng đồi núi nước ta chủ yếu do",
        options: {
          A: "địa hình dốc, mưa lớn theo mùa, mất rừng, tầng đất dày.",
          B: "khí hậu nhiệt đới, địa hình núi dốc, mất lớp phủ thực vật.",
          C: "địa hình cắt xẻ, đất dễ bị thoái hóa, canh tác không hợp lí.",
          D: "địa hình cao, dốc mạnh, mất lớp phủ thực vật, mưa nhiều."
        },
        correctAnswer: "A"
      },
      {
        id: 53,
        question: "Địa hình cacxto khá phổ biến ở nước ta là do tác động kết hợp của các yếu tố nào sau đây?",
        options: {
          A: "Địa hình dốc, mất lớp phủ thực vật, khí hậu nóng.",
          B: "Bề mặt đá vôi, khí hậu nóng, mất lớp phủ thực vật.",
          C: "Khí hậu khô nóng, mưa nhỏ, mất lớp phủ thực vật.",
          D: "Bề mặt đá vôi, khí hậu nhiệt đới, có lượng mưa lớn."
        },
        correctAnswer: "D"
      },
      {
        id: 54,
        question: "Địa hình cacxtơ phổ biến ở vùng núi Đông Bắc nước ta chủ yếu do",
        options: {
          A: "khí hậu nhiệt đới ẩm gió mùa, vỏ phong hóa dày và vụn bở.",
          B: "khí hậu nóng, mưa nhiều và phân mùa, nhiều khối núi đá vôi.",
          C: "có một mùa đông lạnh, tổng lượng mưa lớn, nhiều sơn nguyên.",
          D: "mạng lưới sông ngòi dày đặc, nền nhiệt độ cao và mưa nhiều."
        },
        correctAnswer: "B"
      },
      {
        id: 55,
        question: "Sông ngòi của nước ta có",
        options: {
          A: "chế độ nước theo mùa, ổn định.",
          B: "mật độ sông đều khắp cả nước.",
          C: "lượng phù sa thay đổi trong năm.",
          D: "chế độ nước theo sát chế độ nhiệt."
        },
        correctAnswer: "C"
      },
      {
        id: 56,
        question: "Biểu hiện tính chất nhiệt đới ẩm gió mùa của sông ngòi ở nước ta là",
        options: {
          A: "mạng lưới dày đặc, nhiều phù sa.",
          B: "chế độ nước theo mùa, độ dốc lớn.",
          C: "lưu lượng nước lớn, sông nhỏ ngắn.",
          D: "hướng sông đa dạng, có mùa lũ - cạn."
        },
        correctAnswer: "A"
      },
      {
        id: 57,
        question: "Chế độ nước sông ở nước ta không có đặc điểm nào sau đây?",
        options: {
          A: "Đỉnh lũ trùng tháng mưa cực đại.",
          B: "Dòng chảy diễn biến thất thường.",
          C: "Mùa cạn ứng với mùa gió tây nam.",
          D: "Theo sát sự thay đổi của lượng mưa."
        },
        correctAnswer: "C"
      },
      {
        id: 58,
        question: "Phát biểu nào sau đây đúng về tác động của khí hậu đến sông ngòi nước ta?",
        options: {
          A: "Lượng nước lớn và có sự phân mùa rõ rệt.",
          B: "Phân bố rộng khắp và hướng khác nhau.",
          C: "Mạng lưới dày đặc và nhiều thác ghềnh.",
          D: "Hàm lượng phù sa lớn và sông ngắn dốc."
        },
        correctAnswer: "A"
      },
      {
        id: 59,
        question: "Các nhân tố chủ yếu làm cho nước ta có mật độ sông ngòi lớn là",
        options: {
          A: "khí hậu nhiệt đới gió mùa, rừng bị suy giảm.",
          B: "đồi núi rộng khắp, nhiều hồ đầm, mưa lớn.",
          C: "nhiệt độ cao, mưa nhiều trên sườn núi dốc.",
          D: "vỏ phong hóa vụn bở, nguồn nước đa dạng."
        },
        correctAnswer: "C"
      },
      {
        id: 60,
        question: "Sông ngòi nước ta có hàm lượng phù sa lớn chủ yếu do",
        options: {
          A: "địa hình dốc, mưa lớn theo mùa, vỏ phong hóa vụn bở.",
          B: "thiên tai xảy ra nhiều, địa hình dốc, mất lớp phủ thực vật.",
          C: "khí hậu nhiệt đới ẩm, tàn phá rừng, canh tác thiếu hợp lí.",
          D: "cấu trúc địa hình núi, lượng mưa lớn, đất dễ bị xói mòn."
        },
        correctAnswer: "A"
      },
      {
        id: 61,
        question: "Sông ngòi ở ven biển miền Trung có một mùa lũ chủ yếu do",
        options: {
          A: "lãnh thổ hẹp ngang, gió hướng đông bắc và tây nam, bão.",
          B: "Tín phong bán cầu Bắc, gió tây nam, áp thấp nhiệt đới, bão.",
          C: "gió mùa Tây Nam, dải hội tụ nhiệt đới, gió đông bắc và bão.",
          D: "gió mùa Tây Nam, Tín phong Đông Bắc, địa hình, giáp biển."
        },
        correctAnswer: "C"
      },
      {
        id: 62,
        question: "Sông ngòi nước ta có tổng lượng nước lớn và thay đổi trong năm chủ yếu do",
        options: {
          A: "mưa lớn theo mùa, các hồ tự nhiên tích trữ nước, chiều dài sông lớn.",
          B: "lượng mưa lớn, tập trung theo mùa, nguồn nước từ ngoài lãnh thổ.",
          C: "nước ngầm phong phú, diện tích lưu vực rộng, gió mùa hoạt động.",
          D: "các hồ đầm, rừng tự nhiên điều tiết dòng chảy, khí hậu phân mùa."
        },
        correctAnswer: "B"
      },
      {
        id: 63,
        question: "Sông ngòi nước ta có mạng lưới dày đặc và nhiều nước chủ yếu do",
        options: {
          A: "khí hậu nhiệt đới gió mùa, rừng bị suy giảm, địa hình bị cắt xẻ mạnh.",
          B: "đồi núi rộng khắp, lượng mưa lớn, sông có nhiều phụ lưu và chi lưu.",
          C: "nhiệt ẩm cao, mưa tập trung trên sườn núi dốc, nguồn nước đa dạng.",
          D: "vỏ phong hóa bở rời, lượng mưa lớn theo mùa, thảm thực vật hạn chế."
        },
        correctAnswer: "C"
      },
      {
        id: 64,
        question: "Ở nước ta, quá trình feralit diễn ra mạnh ở vùng",
        options: {
          A: "núi cao.",
          B: "đồng bằng.",
          C: "cửa sông lớn.",
          D: "đồi núi thấp."
        },
        correctAnswer: "D"
      },
      {
        id: 65,
        question: "Đất feralit ở nước ta có đặc điểm",
        options: {
          A: "tầng đất mỏng, không bị chua.",
          B: "chua, nhiều oxit sắt, oxit nhôm.",
          C: "chứa oxit nhôm, tầng đất mỏng.",
          D: "không bị chua, tầng đất rất dày."
        },
        correctAnswer: "B"
      },
      {
        id: 66,
        question: "Đất feralit của nước ta có tầng đất dày chủ yếu do",
        options: {
          A: "đất quá chặt, việc thoát nước kém.",
          B: "quá trình phong hóa diễn ra mạnh.",
          C: "có chứa nhiều oxit sắt và oxit nhôm.",
          D: "mưa nhiều rửa trôi chất badơ dễ tan."
        },
        correctAnswer: "B"
      },
      {
        id: 67,
        question: "Đất feralit ở nước ta có màu đỏ vàng chủ yếu do",
        options: {
          A: "hình thành trên đá mẹ nhiều ba-do.",
          B: "quá trình phong hóa diễn ra mạnh.",
          C: "có chứa nhiều oxit sắt và oxit nhôm.",
          D: "hàm lượng mùn tích tụ trong đất lớn."
        },
        correctAnswer: "C"
      },
      {
        id: 68,
        question: "Quá trình feralit là quá trình hình thành đất chính ở nước ta chủ yếu do",
        options: {
          A: "khí hậu nhiệt đới ẩm gió mùa, địa hình đồi núi thấp rộng.",
          B: "quá trình rửa trôi diễn ra nhanh, nhiều hệ thống sông lớn.",
          C: "vỏ phong hóa dày và vụn bở, diện tích rừng tự nhiên thấp.",
          D: "mưa nhiều trên sườn núi dốc, nền nhiệt độ cao, đá mẹ axit."
        },
        correctAnswer: "A"
      },
      {
        id: 69,
        question: "Đất feralit phân bố rộng khắp ở vùng đồi núi thấp chủ yếu do",
        options: {
          A: "đồi núi thấp rộng, nhiệt độ trung bình năm cao, mưa nhiều.",
          B: "mưa nhiều, nhiệt ẩm cao, nhiều đá mẹ axit ở đồi núi rộng.",
          C: "phong hóa mạnh, có đá mẹ axit, khí hậu nhiệt đới nóng ẩm.",
          D: "có nhiều đá khác nhau, hai mùa mưa và khô, nhiệt độ cao."
        },
        correctAnswer: "B"
      },
      {
        id: 70,
        question: "Cảnh quan đặc trưng cho thiên nhiên nhiệt đới ẩm gió mùa nước ta là hệ sinh thái",
        options: {
          A: "cây bụi gai nhiệt đới khô trên đất cát.",
          B: "rừng ngập mặn trên đất mặn ven biển.",
          C: "rừng gió mùa thường xanh trên đá vôi.",
          D: "rừng nhiệt đới ẩm gió mùa ở đất feralit."
        },
        correctAnswer: "D"
      },
      {
        id: 71,
        question: "Sinh vật nước ta có",
        options: {
          A: "thành phần loài kém đa dạng.",
          B: "ít chịu tác động của con người.",
          C: "loài nhiệt đới chiếm tỉ lệ cao.",
          D: "chỉ có rừng nhiệt đới gió mùa."
        },
        correctAnswer: "C"
      },
      {
        id: 72,
        question: "Sinh vật nước ta",
        options: {
          A: "suy giảm ở một số loài, tập trung toàn bộ ở đồi núi.",
          B: "phân hóa trong không gian, có nhiều thảm thực vật.",
          C: "có nhiều loài sinh vật, rừng nguyên sinh còn nhiều.",
          D: "loài cận nhiệt chiếm ưu thế, nhiều rừng ngập mặn."
        },
        correctAnswer: "B"
      },
      {
        id: 73,
        question: "Hệ sinh thái rừng nhiệt đới gió mùa là cảnh quan tiêu biểu cho thiên nhiên nước ta chủ yếu do",
        options: {
          A: "đồi núi rộng, khí hậu nhiệt đới gió mùa, đất đa dạng.",
          B: "khí hậu nóng ẩm, mưa theo mùa, đất feralit rộng khắp.",
          C: "sinh vật nhiệt đới, nhiệt độ cao, nhiều cao nguyên lớn.",
          D: "lượng mưa lớn, đồng bằng rộng, đất phù sa màu mỡ."
        },
        correctAnswer: "B"
      },
      {
        id: 74,
        question: "Thành phần các loài nhiệt đới chiếm ưu thế ở nước ta chủ yếu do",
        options: {
          A: "vị trí nội chí tuyến, khí hậu nhiệt đới ẩm gió mùa, địa hình thấp.",
          B: "nhiệt độ cao và ổn định, lượng mưa lớn, vị trí giáp biển nhiệt đới.",
          C: "đồi núi thấp trải rộng, động vật nhiệt đới di cư đến, khí hậu nóng.",
          D: "vị trí trong vùng nhiệt đới, đồng bằng châu thổ, sông ngòi dày đặc."
        },
        correctAnswer: "A"
      },
      {
        id: 75,
        question: "Sinh vật nước ta phong phú và đa dạng do tác động kết hợp của",
        options: {
          A: "sông ngòi, đất đai, sự thay đổi nhiệt độ và chế độ mưa.",
          B: "vị trí địa lí, địa hình, đất đai và sự phân hóa của khí hậu.",
          C: "vị trí nội chí tuyến, đất đai, sông ngòi và địa hình đa dạng.",
          D: "địa hình núi, sông ngòi, đất đai và ảnh hưởng của gió mùa."
        },
        correctAnswer: "B"
      },
      {
        id: 76,
        question: "Nhiệt độ cao, lượng mưa lớn của khí hậu nước ta tạo thuận lợi cho",
        options: {
          A: "phát triển lúa nước với năng suất cao.",
          B: "trồng chè chuyên canh sản lượng lớn.",
          C: "phát triển cây dược liệu và rau ôn đới.",
          D: "phát triển vụ đông ở các nơi phía bắc."
        },
        correctAnswer: "A"
      },
      {
        id: 77,
        question: "Hoạt động đánh bắt, nuôi trồng thuỷ sản nước ta có điều kiện phát triển do có",
        options: {
          A: "mạng lưới sông ngòi nhiều nước.",
          B: "khí hậu phân hoá rõ rệt theo mùa.",
          C: "quá trình xâm thực diễn ra mạnh.",
          D: "lượng phù sa của sông ngòi nhiều."
        },
        correctAnswer: "A"
      },
      {
        id: 78,
        question: "Sản xuất nông nghiệp nước ta có tính bấp bênh là do",
        options: {
          A: "thời tiết cực đoan, nhiệt cao.",
          B: "thiên tai, nguy cơ dịch bệnh.",
          C: "ngập lụt, xâm nhập mặn sâu.",
          D: "nhiều bão, mùa khô sâu sắc."
        },
        correctAnswer: "B"
      },
      {
        id: 79,
        question: "Hoạt động du lịch biển ở nước ta chịu ảnh hưởng trực tiếp của",
        options: {
          A: "thiên tai, sự phân mùa khí hậu.",
          B: "cấu trúc địa hình, chế độ mưa.",
          C: "sóng biển, số lượng đảo xa bờ.",
          D: "dòng hải lưu, các loại gió mùa."
        },
        correctAnswer: "A"
      },
      {
        id: 80,
        question: "Ở nước ta, mùa khô là thời kì thuận lợi cho hoạt động",
        options: {
          A: "giao thông đường sông, xuất khẩu.",
          B: "xây dựng công trình, khai thác than.",
          C: "trồng cây lâu năm, chống cháy rừng.",
          D: "thủy điện, chế biến các loại nông sản."
        },
        correctAnswer: "B"
      }
    ]
  },
{
  "quizId": "dia-li-chu-de-12",
  "title": "CHỦ ĐỀ 12: MỘT SỐ HÌNH THỨC TỔ CHỨC LÃNH THỔ NÔNG NGHIỆP",
  "timeMinutes": 40,
  "questions": [
    {
      "id": 1,
      "question": "Hình thức tổ chức lãnh thổ nông nghiệp của nước ta không phải là",
      "options": {
        "A": "trang trại.",
        "B": "khu chế xuất.",
        "C": "vùng chuyên canh.",
        "D": "vùng nông nghiệp."
      },
      "correctAnswer": "B"
    },
    {
      "id": 2,
      "question": "Hình thức tổ chức lãnh thổ nông nghiệp cao nhất ở nước ta hiện nay là",
      "options": {
        "A": "trang trại.",
        "B": "vùng chuyên canh.",
        "C": "khu công nghiệp.",
        "D": "vùng nông nghiệp."
      },
      "correctAnswer": "D"
    },
    {
      "id": 3,
      "question": "Loại hình trang trại chủ yếu ở nước ta hiện nay là",
      "options": {
        "A": "trang trại trồng trọt.",
        "B": "trang trại chăn nuôi.",
        "C": "trang trại lâm nghiệp.",
        "D": "trang trại nuôi trồng thuỷ sản."
      },
      "correctAnswer": "B"
    },
    {
      "id": 4,
      "question": "Các trang trại của nước ta hiện nay tập trung nhiều nhất ở",
      "options": {
        "A": "Tây Nguyên.",
        "B": "Đông Nam Bộ.",
        "C": "Đồng bằng sông Hồng.",
        "D": "Đồng bằng sông Cửu Long."
      },
      "correctAnswer": "C"
    },
    {
      "id": 5,
      "question": "Vùng chuyên canh cây lương thực ở nước ta tập trung tại",
      "options": {
        "A": "đồng bằng.",
        "B": "trung du.",
        "C": "miền núi.",
        "D": "hải đảo."
      },
      "correctAnswer": "A"
    },
    {
      "id": 6,
      "question": "Các trang trại ở nước ta hiện nay",
      "options": {
        "A": "tập trung chủ yếu tại Tây Nguyên.",
        "B": "chỉ phục vụ thị trường nước ngoài.",
        "C": "chưa có trong lĩnh vực ngư nghiệp.",
        "D": "tạo ra nguyên liệu cho công nghiệp."
      },
      "correctAnswer": "D"
    },
    {
      "id": 7,
      "question": "Trang trại ở nước ta hiện nay",
      "options": {
        "A": "không phụ thuộc điều kiện tự nhiên.",
        "B": "chỉ xuất hiện ở khu vực đồng bằng.",
        "C": "đã ứng dụng nhiều công nghệ mới.",
        "D": "còn mang tính chất tự cung, tự cấp."
      },
      "correctAnswer": "C"
    },
    {
      "id": 8,
      "question": "Phát biểu nào sau đây không đúng về trang trại ở nước ta hiện nay?",
      "options": {
        "A": "Số lượng ngày càng tăng, tập trung ở vùng núi.",
        "B": "Chủ yếu là trang trại trồng trọt, sản lượng tăng.",
        "C": "Tạo nhiều hàng hoá, gắn sản xuất với chế biến.",
        "D": "Quy mô sản xuất lớn, ứng dụng kĩ thuật hiện đại."
      },
      "correctAnswer": "B"
    },
    {
      "id": 9,
      "question": "Vùng chuyên canh ở nước ta hiện nay",
      "options": {
        "A": "tạo ra khối lượng sản phẩm lớn.",
        "B": "phân bố toàn bộ ở vùng đồi núi.",
        "C": "chưa có đối với ngành thủy sản.",
        "D": "hình thành 7 vùng trên cả nước."
      },
      "correctAnswer": "A"
    },
    {
      "id": 10,
      "question": "Các vùng chuyên canh của nước ta hiện nay",
      "options": {
        "A": "chỉ tập trung tạo sản phẩm trong ngành trồng trọt.",
        "B": "được phân bố đồng đều giữa các địa phương, vùng.",
        "C": "là hình thức tổ chức lãnh thổ nông nghiệp cao nhất.",
        "D": "cho phép khai thác tốt điều kiện sinh thái mỗi vùng."
      },
      "correctAnswer": "D"
    },
    {
      "id": 11,
      "question": "Một trong những đặc trưng của vùng chuyên canh ở nước ta là",
      "options": {
        "A": "chủ yếu sản xuất nhỏ lẻ, manh mún.",
        "B": "tập trung vào một, vài loại cây trồng.",
        "C": "phần lớn dùng lao động trình độ cao.",
        "D": "quy mô giống nhau giữa các khu vực."
      },
      "correctAnswer": "B"
    },
    {
      "id": 12,
      "question": "Phát biểu nào sau đây không đúng về đặc điểm của vùng chuyên canh ở nước ta?",
      "options": {
        "A": "Tăng cường xuất khẩu nông sản.",
        "B": "Gắn sản xuất với khâu chế biến.",
        "C": "Có hiệu quả kinh tế - xã hội cao.",
        "D": "Tập trung vào chế biến nông sản."
      },
      "correctAnswer": "D"
    },
    {
      "id": 13,
      "question": "Vùng nông nghiệp ở nước ta có nhiều điểm tương đồng về",
      "options": {
        "A": "đặc điểm địa hình, nguồn nước, trình độ dân trí.",
        "B": "mật độ dân số, cơ sở hạ tầng, trình độ thâm canh.",
        "C": "điều kiện sinh thái nông nghiệp, kinh tế - xã hội.",
        "D": "trình độ lao động, diện tích lãnh thổ, thổ nhưỡng."
      },
      "correctAnswer": "C"
    },
    {
      "id": 14,
      "question": "Phát biểu nào sau đây không đúng về các vùng nông nghiệp ở nước ta hiện nay?",
      "options": {
        "A": "Lãnh thổ nước ta hiện có 07 vùng sinh thái nông nghiệp.",
        "B": "Mỗi vùng nông nghiệp có hướng chuyên môn hóa riêng.",
        "C": "Là hình thức tổ chức lãnh thổ nông nghiệp cấp cao nhất.",
        "D": "Được quy hoạch gắn liền với các vùng kinh tế trọng điểm."
      },
      "correctAnswer": "D"
    },
    {
      "id": 15,
      "question": "Điều kiện sinh thái của vùng nông nghiệp Trung du và miền núi Bắc Bộ là",
      "options": {
        "A": "có đất feralit đỏ vàng, khí hậu phân hoá đa dạng.",
        "B": "khí hậu cận xích đạo, đất xám phù sa cổ rộng lớn.",
        "C": "đồng bằng châu thổ rộng lớn, có mùa đông lạnh.",
        "D": "diện tích đất nhiễm mặn nhiều, nền nhiệt độ cao."
      },
      "correctAnswer": "A"
    },
    {
      "id": 16,
      "question": "Vùng nông nghiệp Đồng bằng sông Hồng có điều kiện trồng lúa chất lượng cao chủ yếu do có",
      "options": {
        "A": "mật độ dân số cao, có mùa đông lạnh.",
        "B": "đồng bằng châu thổ, nhiệt ẩm dồi dào.",
        "C": "lao động có kinh nghiệm, diện tích lớn.",
        "D": "sông ngòi nhiều nước, mùa đông lạnh."
      },
      "correctAnswer": "B"
    },
    {
      "id": 17,
      "question": "Vùng nông nghiệp Bắc Trung Bộ trồng được nhiều loại cây công nghiệp chủ yếu do có",
      "options": {
        "A": "đất phù sa, khí hậu có một mùa đông lạnh.",
        "B": "địa hình phân hoá, dân cư tập trung đông.",
        "C": "đất feralit và đất pha cát, khí hậu thích hợp.",
        "D": "cao nguyên badan rộng lớn, đất cát ven biển."
      },
      "correctAnswer": "C"
    },
    {
      "id": 18,
      "question": "Vùng nông nghiệp Duyên hải Nam Trung Bộ có điều kiện phát triển nuôi trồng thuỷ sản chủ yếu do có",
      "options": {
        "A": "ít thiên tai xảy ra, nhiều loài thuỷ sản.",
        "B": "nguồn lợi phong phú, bờ biển trải dài.",
        "C": "nhiều vũng, vịnh, khí hậu cận xích đạo.",
        "D": "đồng bằng ven biển, nhiều ngư trường."
      },
      "correctAnswer": "C"
    },
    {
      "id": 19,
      "question": "Vùng nông nghiệp Tây Nguyên chuyên môn hoá cây cà phê do có điều kiện thuận lợi về",
      "options": {
        "A": "đất badan rộng lớn, khí hậu cận xích đạo.",
        "B": "giao thông khá thuận lợi, nhiều sông lớn.",
        "C": "nhiều cao nguyên xếp tầng, khí hậu nóng.",
        "D": "nguồn nước đảm bảo, chế biến phát triển."
      },
      "correctAnswer": "A"
    },
    {
      "id": 20,
      "question": "Điều kiện sinh thái của vùng nông nghiệp Đông Nam Bộ tạo điều kiện thuận lợi cho phát triển",
      "options": {
        "A": "cây công nghiệp lâu năm, lúa cao sản.",
        "B": "cây công nghiệp nhiệt đới, cây ăn quả.",
        "C": "chăn nuôi trâu, bò, thuỷ sản nước lạnh.",
        "D": "cây dược liệu, đánh bắt thuỷ sản xa bờ."
      },
      "correctAnswer": "B"
    },
    {
      "id": 21,
      "question": "Hoạt động đánh bắt thuỷ sản ở vùng nông nghiệp Đồng bằng sông Cửu Long được tiến hành chủ yếu ở",
      "options": {
        "A": "các hồ, đầm, mạng lưới sông ngòi kênh rạch.",
        "B": "các bãi triều, đầm phá, ô trũng ở đồng bằng.",
        "C": "mạng lưới sông ngòi, ngư trường trọng điểm.",
        "D": "rừng ngập mặn ven biển, hệ thống kênh rạch."
      },
      "correctAnswer": "C"
    },
    {
      "id": 22,
      "question": "Việc đẩy mạnh trồng trọt theo hình thức trang trại ở nước ta chủ yếu nhằm",
      "options": {
        "A": "tạo nhiều việc làm, mở rộng sản xuất.",
        "B": "nâng cấp hạ tầng, mở rộng thị trường.",
        "C": "gia tăng thu nhập, phát triển chế biến.",
        "D": "tăng lượng hàng hóa, khai thác lợi thế."
      },
      "correctAnswer": "D"
    },
    {
      "id": 23,
      "question": "Mục đích chủ yếu của việc đầu tư phát triển trang trại chăn nuôi ở nước ta hiện nay là",
      "options": {
        "A": "gia tăng sản lượng, khai thác tốt tài nguyên.",
        "B": "tăng hiệu quả sản xuất, đáp ứng thị trường.",
        "C": "nâng sức cạnh tranh, áp dụng kĩ thuật mới.",
        "D": "phát huy tốt nguồn lực, đa dạng sản phẩm."
      },
      "correctAnswer": "B"
    },
    {
      "id": 24,
      "question": "Giải pháp chủ yếu nhằm phát triển mô hình trang trại ở nước ta hiện nay là",
      "options": {
        "A": "gắn với tiêu thụ, ứng dụng công nghệ cao.",
        "B": "đào tạo nhân lực, hoàn thiện cơ sở hạ tầng.",
        "C": "đa dạng hóa sản xuất, đẩy mạnh xuất khẩu.",
        "D": "đầu tư công nghiệp chế biến, tăng diện tích."
      },
      "correctAnswer": "A"
    },
    {
      "id": 25,
      "question": "Việc hình thành các vùng chuyên canh nông nghiệp ở miền núi nước ta nhằm mục đích chủ yếu là",
      "options": {
        "A": "chuyên môn hóa sản xuất, đáp ứng thị trường.",
        "B": "giải quyết việc làm, nâng cao giá trị sản phẩm.",
        "C": "phát triển hàng hóa, nâng cao chất lượng sống.",
        "D": "tạo nhiều nông sản, khai thác tốt các thế mạnh."
      },
      "correctAnswer": "C"
    },
    {
      "id": 26,
      "question": "Việc ứng dụng khoa học công nghệ vào các trang trại ở nước ta góp phần chủ yếu vào việc",
      "options": {
        "A": "mở rộng lãnh thổ sản xuất, nâng cao mức sống.",
        "B": "đáp ứng nhu cầu thị trường, đa dạng sản phẩm.",
        "C": "nâng năng suất lao động, giải quyết tốt việc làm.",
        "D": "nâng cao giá trị nông sản, khai thác tốt tự nhiên."
      },
      "correctAnswer": "D"
    },
    {
      "id": 27,
      "question": "Ý nghĩa chủ yếu của tổ chức lãnh thổ nông nghiệp ở nước ta là",
      "options": {
        "A": "sử dụng hiệu quả điều kiện sinh thái nông nghiệp và kinh tế.",
        "B": "giải quyết việc làm cho lao động, khai thác tốt tài nguyên đất.",
        "C": "nâng cao mức sống, thúc đẩy quá trình chuyển dịch lao động.",
        "D": "ứng dụng công nghệ hiện đại, nâng cao chất lượng cuộc sống."
      },
      "correctAnswer": "A"
    },
    {
      "id": 28,
      "question": "Giải pháp chủ yếu nhằm đẩy mạnh sản xuất nông nghiệp chuyên canh ở nước ta là",
      "options": {
        "A": "hoàn thiện cơ sở hạ tầng, phát triển dịch vụ trồng trọt.",
        "B": "ứng dụng công nghệ mới, gắn với chế biến và dịch vụ.",
        "C": "chuyên môn hóa sản xuất, tăng lượng hàng xuất khẩu.",
        "D": "nâng cao trình độ thâm canh, đào tạo nguồn nhân lực."
      },
      "correctAnswer": "B"
    },
    {
      "id": 29,
      "question": "Giải pháp chủ yếu hạn chế rủi ro trong tiêu thụ sản phẩm của vùng chuyên canh ở nước ta là",
      "options": {
        "A": "đa dạng cây trồng, phát triển thị trường, ứng dụng công nghệ.",
        "B": "tích cực quảng bá, thu hút nguồn đầu tư, tập trung cây chủ lực.",
        "C": "hoàn thiện hạ tầng, mở rộng thị trường, đầu tư vào thâm canh.",
        "D": "tăng quy mô sản xuất, thu hút nguồn lao động, tăng năng suất."
      },
      "correctAnswer": "A"
    },
    {
      "id": 30,
      "question": "Ý nghĩa chủ yếu của việc hình thành vùng chuyên canh cây công nghiệp lâu năm ở nước ta là",
      "options": {
        "A": "phát huy thế mạnh, sản xuất hàng hóa, gắn với công nghiệp.",
        "B": "tăng lượng nông sản, giải quyết việc làm, sản xuất tập trung.",
        "C": "đổi mới trồng trọt, tạo nhiều nguyên liệu, phục vụ xuất khẩu.",
        "D": "phân bố lại sản xuất, tăng hiệu quả kinh tế, đáp ứng nhu cầu."
      },
      "correctAnswer": "A"
    }
  ]
} 
];
